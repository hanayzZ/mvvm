...
const compileUtil = {
  ...
  bind: function (node, vm, exp, dir) {
    ...
    // console.log('updater',updaterFn)
    updaterFn && updaterFn(node, this._getVmVal(vm, exp));

    new Watcher(vm, exp, function(value, oldValue) { // 绑定watcher
...
Watcher.prototype = {
	...
	run: function () {
		...
		this.value = newVal;
		//将newVal，oldVal挂载到MVVM实例上 model
		this.cb.call(this.vm, newVal, oldVal)
	},
	get: function () {
		Dep.target = this;//将当前订阅者指向自己
		let value = this.getter.call(this.vm, this.vm);//出发getter,将自身添加到dep中 
...
MVVM.prototype = {
   ...
   */
	_proxyData: function(key, setter, getter){
		let self = this;
		setter = setter ||
		Object.defineProperty(self, key, {//重写defineProperty方法
			configurable:false,
			enumerable: true,
